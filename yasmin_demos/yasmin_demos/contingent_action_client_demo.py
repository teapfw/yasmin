...
from yasmin_ros import ActionState, ContingentActionState, MonitorState
...
class SimpleMonitorState(MonitorState):
    def __init__(self) -> None:
        super().__init__(
            ...
            timeout=5,  # timeout to wait for messages in seconds
        )

    def monitor_handler(self, blackboard: Blackboard, msg: Empty) -> str:
...
def print_failure(blackboard: Blackboard) -> str:
    yasmin.YASMIN_LOG_INFO("Failed contingency!")
    return SUCCEED
...
def main():
    ...
    sm.add_state(
        ...
        transitions={
            ...
            CANCEL: "outcome4",
            ABORT: "outcome4",
            CONTINGENCY_FAILED: "FAILED_CONTINGENCY_REPORT"
        },
    )
    sm.add_state(
        "PRINTING_RESULT",
    ...
    sm.add_state(
        "FAILED_CONTINGENCY_REPORT",
        CbState([SUCCEED], print_failure),
        transitions={
            SUCCEED: "outcome4",
        },
    )
